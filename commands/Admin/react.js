...
module.exports.run = async (Client, message, args, prefix) => {
    ...
    if(!message.member.hasPermission('ADMINISTRATOR', 'MANAGE_ROLES')) return message.channel.send('You dont have the perms to use this command')

    // gets the role for the reaction
    let reactRole = message.mentions.roles.first()

      // Getting the name for the description of the reaction embed
      let reactDescription = args.slice(1).join(" ")
    ...
    .setDescription(reactDescription) // desc of the embed
